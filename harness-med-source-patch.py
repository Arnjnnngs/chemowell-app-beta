...
const PURPOSE_NUMBERY = /\\d/;
const PURPOSE_FEVERY = /fever|antipyretic|temperature/i;
...
async function fetchPurposeSource(name) {
  ...
  try {
    ...
    return { text: text, url: String(found.url), label: MED_SOURCE.label, fetchedAt: Date.now() };
  } catch (e) {
    return null;
  } finally {
...
async function refreshPurposeSource(id, name) {
  ...
  // Gone, renamed, or already described by the caregiver while the request was in flight: in every
  // one of those cases the answer is now stale and is dropped rather than written.

...
// \\p{Nd} IS THE POINT, not the `u` flag on its own. An earlier comment here said the flag was what
// caught non-ASCII digits; measured, \\d is false on \u0662 with it and without it -- the flag only
// makes \\p{Nd} available, and that is what actually matches every decimal digit there is. Listing
// ranges by hand caught Arabic and Devanagari and missed Thai and fullwidth. A guard whose whole job
// is to stop a dose reaching the screen should not be a list somebody has to keep extending.
const PURPOSE_NUMBERY = /\\p{Nd}/u;
...
function purposeSourceLink(med) {
  ...
  // THE LABEL IS ONLY THE RECORD'S WHERE THE RECORD IS BEING USED. Read unconditionally it captioned
  // a URL it did not describe: a stored label of "Drugs.com" printed "Look it up on Drugs.com" over a
  // medlineplus.gov search. Unreachable before this release; the rename path is a new route to it.
  const usable = purposeSourceMatchesName(med);
  const label = (usable && src && String(src.label || '').trim()) || MED_SOURCE.label;
  const shown = purposeOf(med);
  const sourced = sourcedPurposeText(med);
  const usingSourceText = !!sourced && shown === sourced;
  if (usingSourceText && src && src.url && usable) {

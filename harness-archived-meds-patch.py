...
rep("""  if (doses && doses.length) medication.doses = doses; else delete medication.doses;""",
  ...
  const awaySpans = (Array.isArray(original.awayPeriods) ? original.awayPeriods : [])
    ...
    .filter(span => isFinite(span.start) && isFinite(span.end)
      ...
      // WHAT A SPAN DOES CHANGE in THIS app is every surface fed by missedDosesFor(): the banner,
      // the card's MISSED label, Today's journal, and the History rows and day summaries -- where a
      // day can stop appearing at all, because a History day here exists only if it has misses.
      // IT DOES NOT CHANGE THE EXPORT IN THIS APP. buildExportRows() is allEntriesRaw().filter().map()
      // and never calls missedDosesFor(); there are no derived missed-dose rows here to drop. The
      // sibling app HAS them and its copy of this comment says so; a previous version of this one
      // named the export and quoted the sibling's fixture numbers, which is the same
      // write-it-because-it-reads-well defect in the opposite direction.
      // Do not write a reassurance here that has not been measured IN THIS FILE.
      && span.end <= Date.now());
